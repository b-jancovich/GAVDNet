function [metrics, falsePositives, falseNegatives] = compareDetectionsToSubsampledTestDatasetCTBTO(groundtruthPath, inferenceResultsPath, detectionTolerance, maxDetectionDuration)
% compareDetectionsToSubsampledTestDatasetCTBTO
% Compare GAVDNet detector results with ground truth (detecions from 
% CTBTO data) using Hungarian algorithm via matchpairs and calculate 
% performance metrics.
%
% Assumes inference results do not contain 'failComment' for filtering.
%
% Inputs:
%   groundtruthPath      - Path to ground truth .mat file (containing 'testDatasetDetectionsList' table)
...
%
% Ben Jancovich, 2025 (Updated with simplifications, diagnostics, and removal of failComment logic)
...
if diag_missingEventStartTimeField > 0, fprintf('  %d entries: Missing ''eventStartTime'' field.\n', diag_missingEventStartTimeField); end
if diag_emptyEventStartTime > 0, fprintf('  %d entries: ''eventStartTime'' was empty (e.g., 0x0 datetime).\n', diag_emptyEventStartTime); end
if diag_nonScalarOrNonDatetimeEventStartTime > 0, fprintf('  %d entries: ''eventStartTime'' not scalar datetime.\n', diag_nonScalarOrNonDatetimeEventStartTime); end
if diag_NaTEventStartTime > 0, fprintf('  %d entries: ''eventStartTime'' was NaT (Not-a-Time), resulting in NaN time.\n', diag_NaTEventStartTime); end
if diag_missingConfidenceField > 0, fprintf('  %d entries: Missing ''confidence'' field.\n', diag_missingConfidenceField); end
if diag_emptyConfidence > 0, fprintf('  %d entries: ''confidence'' was empty.\n', diag_emptyConfidence); end
if diag_nonNumericOrNonScalarConfidence > 0, fprintf('  %d entries: ''confidence'' not numeric scalar.\n', diag_nonNumericOrNonScalarConfidence); end
...
if numResultsDetections_Evaluated == 0 && numGroundtruthDetections == 0
    ...
    falsePositives = struct('DetectionStartTime', {}, 'DetectionEndTime', {}, 'DetectionStartSamp', [], 'DetectionEndSamp', [], 'AudioFilename', {}, 'AudioFs', [], 'Confidence', []);
...
if numFalsePositives > 0
    ...
    fpFieldNames = {'DetectionStartTime', 'DetectionEndTime', 'DetectionStartSamp', 'DetectionEndSamp', 'AudioFilename', 'AudioFs', 'Confidence'};
    ...
    for i = 1:fpCount 
        ...
        if isfield(currentResultStruct, 'eventStartTime') && isdatetime(currentResultStruct.eventStartTime) && isscalar(currentResultStruct.eventStartTime)
            falsePositives(i).DetectionStartTime = currentResultStruct.eventStartTime; else falsePositives(i).DetectionStartTime = NaT; end
        if isfield(currentResultStruct, 'eventEndTime') && isdatetime(currentResultStruct.eventEndTime) && isscalar(currentResultStruct.eventEndTime)
            falsePositives(i).DetectionEndTime = currentResultStruct.eventEndTime; else falsePositives(i).DetectionEndTime = NaT; end
        if isfield(currentResultStruct, 'eventSampleStart') && isnumeric(currentResultStruct.eventSampleStart) && isscalar(currentResultStruct.eventSampleStart)
            falsePositives(i).DetectionStartSamp = currentResultStruct.eventSampleStart; else falsePositives(i).DetectionStartSamp = NaN; end
        if isfield(currentResultStruct, 'eventSampleEnd') && isnumeric(currentResultStruct.eventSampleEnd) && isscalar(currentResultStruct.eventSampleEnd)
            falsePositives(i).DetectionEndSamp = currentResultStruct.eventSampleEnd; else falsePositives(i).DetectionEndSamp = NaN; end
        if isfield(currentResultStruct, 'fileName') && ischar(currentResultStruct.fileName)
            falsePositives(i).AudioFilename = currentResultStruct.fileName; else falsePositives(i).AudioFilename = ''; end
        if isfield(currentResultStruct, 'fileFs') && isnumeric(currentResultStruct.fileFs) && isscalar(currentResultStruct.fileFs)
            falsePositives(i).AudioFs = currentResultStruct.fileFs; else falsePositives(i).AudioFs = NaN; end
        if isfield(currentResultStruct, 'confidence') && isnumeric(currentResultStruct.confidence) && isscalar(currentResultStruct.confidence)
            falsePositives(i).Confidence = currentResultStruct.confidence; else falsePositives(i).Confidence = NaN; end
    end
...
if totalAudioDuration <= 0 && isfield(groundtruthData, 'testDatasetFileList')
    ...
    % Since failedFiles logic is removed from results, no filtering of gtTableForDuration here based on it
