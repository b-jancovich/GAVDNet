...
for i = 1:fileSampleSize
    ...
    % Compute spectrogram
    % NOTE: FFTLen calculation is a user-specified choice for high interpolation.
...
% --- FIX: Extract nested fields using arrayfun ---
...
% V1.0 - incorrect handling of file-level percentiles to dataset level
% percentiles.
% function [datasetStats, fileStats] = measureDatasetDynamicRange(audioFolderPath, fileSampleSize, ...
...
% %   audioFolderPath = path to folder containing audio files
% %   fileSampleSize  = number of files to randomly sample and analyze
% %   fsIn            = original sampling rate of audio files (Hz)
% %   fsTarget        = target sampling rate for analysis (Hz)  
% %   bandwidth       = [min, max] frequency range of interest (Hz)
% %   windowLen       = STFT window length (samples)
% %   hopLen          = STFT hop length (samples)
...
%     audioFolderPath {validateattributes(audioFolderPath,{'char','string'},{'nonempty'},'measureDatasetDynamicRange','audioFolderPath')}
%     fileSampleSize {validateattributes(fileSampleSize,{'single','double'},{'nonempty','scalar','real','finite','positive','integer'},'measureDatasetDynamicRange','fileSampleSize')}
%     fsIn {validateattributes(fsIn,{'single','double'},{'nonempty','scalar','real','finite','positive'},'measureDatasetDynamicRange','fsIn')}
%     fsTarget {validateattributes(fsTarget,{'single','double'},{'nonempty','scalar','real','finite','positive'},'measureDatasetDynamicRange','fsTarget')}
%     bandwidth {validateattributes(bandwidth,{'single','double'},{'nonempty','vector','real','finite','positive'},'measureDatasetDynamicRange','bandwidth')}
%     windowLen {validateattributes(windowLen,{'single','double'},{'nonempty','scalar','real','finite','positive'},'measureDatasetDynamicRange','windowLen')}
%     hopLen {validateattributes(hopLen,{'single','double'},{'nonempty','scalar','real','finite','positive'},'measureDatasetDynamicRange','hopLen')}
...
% fprintf('Scanning audio folder: %s\n', audioFolderPath);
% audioExtensions = {'.wav', '.flac', '.mp3', '.m4a', '.ogg'};
% audioFiles = [];
% 
% for ext = audioExtensions
%     files = dir(fullfile(audioFolderPath, ['*' ext{1}]));
...
% end
% 
% % Initialize storage for file-level statistics
% fileStats = struct();
% allSpectrograms = cell(fileSampleSize, 1);
% 
% % Process each file
% fprintf('\nProcessing files:\n');
% for i = 1:fileSampleSize
...
%         spectMeldB = spectrogramToMeldB(spect, fsTarget, FFTLen, bandwidth);
% 
%         % Store spectrogram for later aggregate analysis
%         allSpectrograms{i} = spectMeldB;
% 
%         % Calculate file-level statistics
%         fileStats(i).filename = sampledFiles(i).name;
%         fileStats(i).duration = size(spectMeldB,2) * hopLen / fsTarget;
...
% 
%         % Outlier analysis
%         fileStats(i).outliers.extremeHigh = sum(spectMeldB(:) > fileStats(i).percentiles(end-1)); % Above 99th percentile
%         fileStats(i).outliers.extremeLow = sum(spectMeldB(:) < fileStats(i).percentiles(2)); % Below 1st percentile
%         fileStats(i).outliers.percentExtreme = (fileStats(i).outliers.extremeHigh + fileStats(i).outliers.extremeLow) / numel(spectMeldB) * 100;
% 
%         fprintf('✓\n');
% 
%     catch ME
...
% 
% % Remove any failed files
% validIdx = ~cellfun(@isempty, allSpectrograms);
% fileStats = fileStats(validIdx);
% allSpectrograms = allSpectrograms(validIdx);
% actualFileCount = sum(validIdx);
% 
% fprintf('\nSuccessfully processed %d files.\n', actualFileCount);
% 
...
% globalDRs = zeros(1, length(fileStats));
% robustDRs = zeros(1, length(fileStats));
% globalMaxes = zeros(1, length(fileStats));
% globalMins = zeros(1, length(fileStats));
% amplitudeMeans = zeros(1, length(fileStats));
% amplitudeStds = zeros(1, length(fileStats));
% amplitudeMedians = zeros(1, length(fileStats));
% amplitudeMads = zeros(1, length(fileStats));
% 
% for i = 1:length(fileStats)
%     globalDRs(i) = fileStats(i).globalDynamicRange;
%     robustDRs(i) = fileStats(i).robustDynamicRange;
%     globalMaxes(i) = fileStats(i).globalMax;
%     globalMins(i) = fileStats(i).globalMin;
%     amplitudeMeans(i) = fileStats(i).amplitude.mean;
%     amplitudeStds(i) = fileStats(i).amplitude.std;
%     amplitudeMedians(i) = fileStats(i).amplitude.median;
%     amplitudeMads(i) = fileStats(i).amplitude.mad;
% end
% 
% % Aggregate percentiles across all files
% allPercentiles = vertcat(fileStats.percentiles);
% datasetPercentiles = mean(allPercentiles, 1);
% 
% % Dataset statistics
% datasetStats.numFilesAnalyzed = actualFileCount;
% datasetStats.totalDuration = sum([fileStats.duration]);
...
% datasetStats.amplitudeLevels.globalMin = min(globalMins);
% datasetStats.amplitudeLevels.meanAcrossFiles = mean(amplitudeMeans);
% datasetStats.amplitudeLevels.medianAcrossFiles = mean(amplitudeMedians);
% datasetStats.amplitudeLevels.stdConsistency = std(amplitudeStds);
% datasetStats.amplitudeLevels.madConsistency = std(amplitudeMads);
% 
% % Dataset-wide percentiles
% datasetStats.percentiles.levels = prctiles;
% datasetStats.percentiles.values = datasetPercentiles;
% 
% % Variability analysis
% datasetStats.variability.globalDRCoeff = std(globalDRs) / mean(globalDRs);
% datasetStats.variability.robustDRCoeff = std(robustDRs) / mean(robustDRs);
...
% fprintf('Computing saturation recommendations...\n');
% 
% % Conservative: Use robust range + buffer
% datasetStats.recommendations.saturation.conservative = ceil(mean(robustDRs) + 2*std(robustDRs));
% 
% % Adaptive: Based on percentile coverage
% datasetStats.recommendations.saturation.adaptive95 = ceil(mean(datasetPercentiles(end-2) - datasetPercentiles(3)));
% datasetStats.recommendations.saturation.adaptive99 = ceil(mean(datasetPercentiles(end-1) - datasetPercentiles(2)));
% 
% % Data-driven: Based on actual global ranges
% datasetStats.recommendations.saturation.dataDriven = ceil(prctile(globalDRs, 75)); % 75th percentile of observed ranges
% 
% % Standardization recommendations
% fprintf('Computing standardization recommendations...\n');
% 
% % Robust vs standard standardization analysis
% meanStdRatio = mean(amplitudeStds ./ amplitudeMads) / 1.4826; % How much std is inflated vs MAD
% datasetStats.recommendations.standardization.useRobust = meanStdRatio > 1.5; % Use robust if std is >50% inflated
% 
% if datasetStats.recommendations.standardization.useRobust
%     datasetStats.recommendations.standardization.method = "robust";
% else
...
% 
% % Optimal processing strategy
% if std(globalDRs) / mean(globalDRs) > 0.3 % High variability between files
%     datasetStats.recommendations.processingStrategy = "file-level-normalization";
% else
%     datasetStats.recommendations.processingStrategy = "global-normalization";
% end
% 
% % Display results
% displayDatasetResults(datasetStats, fileStats);
% 
% % Plot results if requested
% if plotResults
%     plotDatasetAnalysis(datasetStats, fileStats, allSpectrograms);
% end
% 
% end
% 
% function displayDatasetResults(datasetStats, fileStats)
% %displayDatasetResults Display comprehensive dataset analysis results
% 
% fprintf('\n');
% fprintf('==========================================\n');
...
% fprintf('\n');
% 
% fprintf('ROBUST DYNAMIC RANGE STATISTICS:\n');
% fprintf('  Mean: %.1f ± %.1f dB\n', datasetStats.robustDynamicRange.mean, datasetStats.robustDynamicRange.std);
% fprintf('  Median: %.1f dB\n', datasetStats.robustDynamicRange.median);
% fprintf('  Range: %.1f - %.1f dB\n', datasetStats.robustDynamicRange.min, datasetStats.robustDynamicRange.max);
% fprintf('  Variability (CoV): %.2f\n', datasetStats.variability.robustDRCoeff);
...
% fprintf('  Mean amplitude across files: %.1f dB\n', datasetStats.amplitudeLevels.meanAcrossFiles);
% fprintf('  Amplitude variability (CoV): %.2f\n', datasetStats.variability.amplitudeLevelCoeff);
% fprintf('  Std inflation factor: %.2f\n', datasetStats.recommendations.standardization.stdInflationFactor);
% fprintf('\n');
% 
% fprintf('==========================================\n');
% fprintf('OPTIMIZATION RECOMMENDATIONS\n');
% fprintf('==========================================\n');
% 
% fprintf('SATURATION PARAMETERS:\n');
% fprintf('  Conservative (robust + 2σ): %d dB\n', datasetStats.recommendations.saturation.conservative);
% fprintf('  Data-driven (75th percentile): %d dB\n', datasetStats.recommendations.saturation.dataDriven);
% fprintf('  Adaptive 95%% coverage: %d dB\n', datasetStats.recommendations.saturation.adaptive95);
% fprintf('  Adaptive 99%% coverage: %d dB\n', datasetStats.recommendations.saturation.adaptive99);
% fprintf('\n');
% 
% fprintf('STANDARDIZATION METHOD:\n');
% fprintf('  Recommended method: %s\n', datasetStats.recommendations.standardization.method);
...
%     fprintf('  Justification: Standard deviation inflated by %.1fx due to outliers\n', ...
%         datasetStats.recommendations.standardization.stdInflationFactor);
%     fprintf('  Use: median + MAD with 1.4826 scaling factor\n');
% else
%     fprintf('  Justification: Data well-behaved, standard methods appropriate\n');
%     fprintf('  Use: mean + standard deviation\n');
% end
% fprintf('\n');
% 
% fprintf('PROCESSING STRATEGY:\n');
% fprintf('  Recommended: %s\n', datasetStats.recommendations.processingStrategy);
% if strcmp(datasetStats.recommendations.processingStrategy, "file-level-normalization")
%     fprintf('  Justification: High variability between files (CoV = %.2f)\n', ...
%         datasetStats.variability.globalDRCoeff);
%     fprintf('  Apply normalization per file, then combine\n');
% else
%     fprintf('  Justification: Consistent amplitude levels across files\n');
%     fprintf('  Global normalization across entire dataset appropriate\n');
% end
% fprintf('\n');
% 
% end
...
% end
% 
% function plotDatasetAnalysis(datasetStats, fileStats, allSpectrograms)
% %plotDatasetAnalysis Create comprehensive visualization of dataset analysis
% 
...
% % Subplot 3: Global vs Robust comparison
% subplot(3,4,3);
% scatter(globalDRs, robustDRs, 50, 'filled')
% xlabel('Global Dynamic Range (dB)');
% ylabel('Robust Dynamic Range (dB)');
% title('Global vs Robust Dynamic Range');
% hold on;
% plot([min([globalDRs, robustDRs]), max([globalDRs, robustDRs])], ...
%      [min([globalDRs, robustDRs]), max([globalDRs, robustDRs])], 'k--', 'LineWidth', 1);
% grid on;
% 
% % Subplot 4: Amplitude level consistency
% subplot(3,4,4);
...
% end
% histogram(amplitudeMeans, 15, 'Normalization', 'probability', 'FaceColor', [0.4 0.8 0.2]);
% title('Amplitude Level Distribution');
% xlabel('Mean Amplitude (dB)');
% ylabel('Probability');
% grid on;
% 
% % Subplot 5: Temporal dynamic range statistics
% subplot(3,4,5);
% temporalMeans = zeros(1, length(fileStats));
% temporalMaxes = zeros(1, length(fileStats));
% for i = 1:length(fileStats)
%     temporalMeans(i) = fileStats(i).temporal.mean;
%     temporalMaxes(i) = fileStats(i).temporal.max;
% end
% scatter(temporalMeans, temporalMaxes, 50, 'filled');
% xlabel('Mean Temporal DR (dB)');
% ylabel('Max Temporal DR (dB)');
% title('Temporal Dynamic Range Patterns');
% grid on;
...
% % Subplot 6: Standardization analysis
% subplot(3,4,6);
% stds = zeros(1, length(fileStats));
% mads = zeros(1, length(fileStats));
% for i = 1:length(fileStats)
%     stds(i) = fileStats(i).amplitude.std;
%     mads(i) = fileStats(i).amplitude.mad;
% end
% scatter(mads * 1.4826, stds, 50, 'filled');
% xlabel('MAD × 1.4826 (dB)');
% ylabel('Standard Deviation (dB)');
% title('Standardization Method Analysis');
% hold on;
% plot([min([mads*1.4826, stds]), max([mads*1.4826, stds])], ...
%      [min([mads*1.4826, stds]), max([mads*1.4826, stds])], 'k--', 'LineWidth', 1);
% grid on;
% 
% % Subplot 7: Outlier analysis
% subplot(3,4,7);
% outlierPercents = zeros(1, length(fileStats));
% for i = 1:length(fileStats)
%     outlierPercents(i) = fileStats(i).outliers.percentExtreme;
% end
% histogram(outlierPercents, 15, 'Normalization', 'probability', 'FaceColor', [0.8 0.2 0.8]);
% title('Outlier Percentage Distribution');
...
% % Subplot 8: Dataset percentiles
% subplot(3,4,8);
% semilogx(datasetStats.percentiles.levels, datasetStats.percentiles.values, 'o-', 'LineWidth', 2);
% title('Dataset Amplitude Percentiles');
% xlabel('Percentile Level');
% ylabel('Amplitude (dB)');
% grid on;
% 
...
%            datasetStats.recommendations.saturation.adaptive95, ...
%            datasetStats.recommendations.saturation.adaptive99];
% recNames = {'Conservative', 'Data-Driven', '95% Coverage', '99% Coverage'};
% bar(satRecs, 'FaceColor', [0.6 0.6 0.6]);
% set(gca, 'XTickLabel', recNames);
% xtickangle(45);
% title('Saturation Recommendations');
% ylabel('Saturation (dB)');
% grid on;
% 
% % Subplot 10: Example spectrogram
% subplot(3,4,10);
% if ~isempty(allSpectrograms)
%     exampleSpect = allSpectrograms{1};
%     imagesc(exampleSpect);
%     axis xy;
%     colorbar;
...
% % Subplot 11: Dynamic range vs file duration
% subplot(3,4,11);
% durations = zeros(1, length(fileStats));
% for i = 1:length(fileStats)
%     durations(i) = fileStats(i).duration / 3600; % Convert to hours
% end
% scatter(durations, globalDRs, 50, 'filled');
% xlabel('File Duration (hours)');
% ylabel('Global Dynamic Range (dB)');
% title('Duration vs Dynamic Range');
% grid on;
...
%     text(0.05, 0.95 - (i-1)*0.1, summaryText{i}, 'FontSize', 10, 'FontWeight', 'bold');
% end
