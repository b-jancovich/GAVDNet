function [metrics, falsePositives, falseNegatives] = compareDetectionsToSubsampledTestDatasetSORP(groundtruthPath, inferenceResultsPath, detectionTolerance, maxDetectionDuration)
% compareDetectionsToSubsampledTestDatasetSORP
% Compare GAVDNet detector results with ground truth (detections from 
% SORP data) using Hungarian algorithm via matchpairs and calculate 
% performance metrics.
%
% Assumes inference results do not contain 'failComment' for filtering.
%
% Inputs:
%   groundtruthPath      - Path to ground truth tab-delimited txt file (SORP format)
...
%% Extract detections from groundtruth (SORP format)
...
gtFs = repmat(firstFileFs, numGroundtruthDetections, 1);
