...
% 
% % Different noiseless sample paths for each version of the CPBW Song detector:
% many_noiseless_sample_paths = "D:\DGS_Chagos_Exemplars\U1 & U2\Denoised\detectionAudio_21560_19-Aug-2005_19_35_36_24.295708_RXDENOISED.wav";
% many_noiseless_sample_paths = [many_noiseless_sample_paths, repelem("D:\DGS_Chagos_Exemplars\U1 & U2\Denoised", 3)]';
% 
% % Different output paths for each version of the CPBW Song detector:
...
for modelNumber = 1:nModels
    ...
    % Get current settings & overwrite config file ones:
    snrRange = snrRangeSweep(modelNumber, :);
    noiseless_sample_path = many_noiseless_sample_paths(modelNumber);
    gavdNetDataPath = many_gavdNetDataPaths{modelNumber};

    fprintf('\tCurrent SNR Range: %d to %d\n', snrRange)
    fprintf('\tCurrent noiseless sample path: %s\n', noiseless_sample_path)
    ...
    nCleanSignals = length(ads_cleanSignals.Files);
    cleanSignalDurations = zeros(1, nCleanSignals);
    i = 1;
    ...
    model.dataSynthesisParams.freqShiftRangeHz = [-max_down_shift_hz, max_up_shift_hz];
    ...
    clearvars -except gpuDeviceID trainDS valDS model gavdNetDataPath configPath ...
        nModels snrRangeSweep many_noiseless_sample_paths many_gavdNetDataPaths modelNumber
    ...
    clearvars -except configPath nModels snrRangeSweep many_noiseless_sample_paths many_gavdNetDataPaths modelNumber
